...
def EXTENDED_MEMOIZED_CUT_ROD(p, n):
    ...
    for i in xrange(0, n+1):    # 初始化r [0 to n]   r的长度是n+1
        r.append(-1)
        s.append(0)
    return EXTENDED_MEMOIZED_CUT_ROD_AUX(p, n, r, s), s
...
def EXTENDED_MEMOIZED_CUT_ROD_AUX(p, n, r, s):
    ...
    if n is 0:
        ...
        for i in xrange(1, n+1):    # 1 to n
            if q < p[i] + EXTENDED_MEMOIZED_CUT_ROD_AUX(p, n-i, r, s):
                q = p[i] + EXTENDED_MEMOIZED_CUT_ROD_AUX(p, n-i, r, s)
                s[n] = i
    r[n]=q
    return q

def PRINT_CUT_ROD_SOLUTION(p, n):
    q, s = EXTENDED_MEMOIZED_CUT_ROD(p, n)
